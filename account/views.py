...
def forgetpassword(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(email=email).first():
            ...
            profile_obj.save()
            send_forget_password_mail(email=email, token=token, request=request)
...
def send_forget_password_mail(email, token, request):
    ...
    reset_password_link = reverse('password_reset_confirm', kwargs={'uidb64': token, 'token': token})
    reset_password_url = f"http://{domain}{reset_password_link}"
    message = f'Hi, click on the link to reset your password: {reset_password_url}'
    email_from = settings.EMAIL_HOST_USER
    recipient_list = [email]
    send_mail(subject, message, email_from, recipient_list)
    return True
